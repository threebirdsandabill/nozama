...
var updateCartArray = function updateCartArray(itemId, itemQty, itemPrice, updateType) {
  var cartItems = store.user.cart;
  var checkIfItemExists = function checkIfItemExists(itemId) {
  ...
  var indexOfArray = checkIfItemExists(itemId);
...
var cartTotal = function cartTotal(data) {
  var totalCost = 0;
  var totalItems = 0;
  var cartItems = store.user.cart;
  for (var i = 0; i < cartItems.length; i++) {
...
var appEvents = __webpack_require__(18);
...
var onShowSignUpModal = function onShowSignUpModal() {
  $('#signUpModal').modal('show');
};

var onDivertSignUpModal = function onDivertSignUpModal() {
  $('#signUpModal').modal('show');
  $('#signInModal').modal('hide');
};

var onSignUp = function onSignUp(event) {
  // TODO I want to make it so that it automatically signs in. I will look into it tonight
  event.preventDefault();
  var data = getFormFields(this);
  api.signUp(data).then(ui.onSignUpSuccess).catch(ui.onSignUpFailure);
};

var onShowSignInModal = function onShowSignInModal() {
...
var onShowChangePasswordModal = function onShowChangePasswordModal() {
...
var onSignOut = function onSignOut(event) {
  // broke in Firefox becuase never passed event
  event.preventDefault();
  var data = getFormFields(this);
  api.signOut(data).then(ui.onSignOutSuccess).then(function () {
    store.user = undefined;
  }).catch(ui.signOutFailure);
};

var onCloseSignUpModal = function onCloseSignUpModal() {
  $('#signUpForm').each(function () {
    this.reset();
  });
};

var onCloseSignInModal = function onCloseSignInModal() {
  $('#signInForm').each(function () {
    this.reset();
  });
};

var onCloseChangePassword = function onCloseChangePassword() {
...
var onSignInFailure = function onSignInFailure() {
  ...
  $('#signInForm').each(function () {
    // added to fix clearing on failure.
    this.reset();
  });
};
...
var orderTotalAmount = void 0;
// const

var onShowItems = function onShowItems() {
  //  event.preventDefault()
  api.getItems().then(ui.getItemsSucces).catch(ui.getItemsFailure);
};

var onAddToCart = function onAddToCart(event) {
  event.preventDefault();
  if (store.user !== undefined) {
    var itemId = $(event.target).data('btnitemid');
    ...
    };api.updateCart(data).then(function (data) {
      ui.updateCartSuccess(data, actionDescription);
    }).then(onGetUserCart).catch(ui.updateCartFailure);
    // api.updateCart()
...
var onUpdateCartItemQty = function onUpdateCartItemQty(event) {
  event.preventDefault();
  // set actionType so that the ui function knows what kind of message to give
  var actionDescription = 'updated';
  // get cart items

  var itemId = $(event.target).data('btnupdateitemid');
  var itemQty = $('#updateqty_' + itemId).val();
  var itemPrice = $('#price_' + itemId).val();
  var data = {
    'user': {
...
var onRemoveCartItem = function onRemoveCartItem(event) {
  event.preventDefault();
  var actionDescription = 'remove';
  // get cart items
  var itemId = $(event.target).data('btnremoveitemid');
  var data = {
    'user': {
  ...
  api.updateCart(data).then(function (d) {
...
var onCartClickOpen = function onCartClickOpen(event) {
  // this broke in Firefox because we didn't pass 'event' as a var.
  event.preventDefault();
  if (store.user !== undefined) {
    api.getUserCart().then(cart.cartTotal).then(ui.populateCart).catch(ui.populateCartError);
  } else {
    $('#signInModal').modal('show');
  }
};

var onGetUserCart = function onGetUserCart() {
  //  event.preventDefault()
  api.getUserCart().then(cart.cartTotal).then(ui.getUserCartSuccess).catch(ui.getUserCartFailure);
};

var onGetTotal = function onGetTotal() {
  event.preventDefault();
  cart.cartTotal();
  $('#addToCartModal').modal('show');
  var showCartHtml = showCartTemplate({ items: store.user.cart });
  $('.cart-populate').html(showCartHtml);
};

var onPurchaseClick = function onPurchaseClick(event) {
...
var getUserDetails = function getUserDetails() {
  authApi.getUser().then(convertCartToOrder);
};

var emptycart = function emptycart() {
  // TODO this isn't working...
  var userData = void 0;
  authApi.getUser().then(function (d) {
    userData = d;
    userData.user.cart = [{}];
...
var showAnEmptyCart = function showAnEmptyCart() {
  api.getUserCart().then(cart.cartTotal).then(ui.getUserCartSuccess).catch(ui.getUserCartFailure);
};

var convertCartToOrder = function convertCartToOrder(data) {
  var userData = {
    order: {
      orderDate: new Date(),
      items: [{}],
  ...
  var orderCost = 0;
  for (var i = 0; i < data.user.cart.length; i++) {
    var itemsInstance = {
  ...
  api.makeOrder(userData).then(ui.makeOrderSuccess).then(showAnEmptyCart)
  // .then(cart.cartTotal)
...
var handler = StripeCheckout.configure({
  ...
  token: function token(_token) {
    ...
    };api.createCharge(data).then(ui.paymentSuccessful).catch(ui.paymentFailure).then(getUserDetails);
...
var displayOrderHistory = function displayOrderHistory() {
  ...
  api.getPurchaseHistory().then(ui.orderHistorySuccess).then('ui order history success promise in events has fired').catch(ui.orderHistoryFailure);
};
...
var makeOrder = function makeOrder(data) {
  return $.ajax({
...
var showItemGrid = __webpack_require__(21);
var showCartTemplate = __webpack_require__(38);
var showCartSummary = __webpack_require__(39);
var orderHistoryTemplate = __webpack_require__(40);
var store = __webpack_require__(2);
var cart = __webpack_require__(8);

var getItemsSucces = function getItemsSucces(data) {
  $('#store-items').html(showItemGrid({ items: data.items }));
};

var convertCartToOrderFailure = function convertCartToOrderFailure() {
  $.toast({
...
var paymentSuccessful = function paymentSuccessful(data) {
...
var paymentFailure = function paymentFailure(data) {
  $.toast({
...
var updateCartSuccess = function updateCartSuccess(data, actionDescription) {
  $.toast({
...
var getUserCartSuccess = function getUserCartSuccess(data) {
  store.user.cart = data.user.cart;
};

var makeOrderSuccess = function makeOrderSuccess(data) {
  $.toast({
...
var orderHistorySuccess = function orderHistorySuccess(data) {
  store.user.orders = data.orders;
  var orderHistoryHtml = orderHistoryTemplate({ orders: data.orders });
  $('.history-populate').html(orderHistoryHtml);
};

var orderHistoryFailure = function orderHistoryFailure(error) {
  console.error(error);
};

module.exports = {
  getItemsSucces: getItemsSucces,
  updateCartSuccess: updateCartSuccess,
  paymentSuccessful: paymentSuccessful,
  paymentFailure: paymentFailure,
  populateCart: populateCart,
  getUserCartSuccess: getUserCartSuccess,
  makeOrderSuccess: makeOrderSuccess,
  orderHistorySuccess: orderHistorySuccess,
  orderHistoryFailure: orderHistoryFailure,
  convertCartToOrderFailure: convertCartToOrderFailure
};
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(0)))
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "      <div class=\"col-sm-6 col-xs-12 col-md-4 item\">\n        <div class=\"card\" data-cardDivID=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n          <div class=\"row\">\n            <div class=\"col-xs-4\">\n              <img src=\"https://s3.amazonaws.com/anna0312wdi/nozama-images/"
    + alias2(((helper = (helper = helpers.image || (depth0 != null ? depth0.image : depth0)) != null ? helper : alias4),(typeof helper === alias5 ? helper.call(alias3,{"name":"image","hash":{},"data":data,"blockParams":blockParams}) : helper)))
    + "\" class=\"item-image-small\">\n            </div>\n            <div class=\"col-xs-8\">\n               <input type=\"hidden\" id=\"price_"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" value=\""
    + alias2(((helper = (helper = helpers.price || (depth0 != null ? depth0.price : depth0)) != null ? helper : alias4),(typeof helper === alias5 ? helper.call(alias3,{"name":"price","hash":{},"data":data,"blockParams":blockParams}) : helper)))
    + "\" >\n              <strong>"
    + alias2(((helper = (helper = helpers.name || (depth0 != null ? depth0.name : depth0)) != null ? helper : alias4),(typeof helper === alias5 ? helper.call(alias3,{"name":"name","hash":{},"data":data,"blockParams":blockParams}) : helper)))
    + "</strong>\n              <br> $"
    + alias2(((helper = (helper = helpers.price || (depth0 != null ? depth0.price : depth0)) != null ? helper : alias4),(typeof helper === alias5 ? helper.call(alias3,{"name":"price","hash":{},"data":data,"blockParams":blockParams}) : helper)))
    + "\n              <br>\n              <div class=\"form-inline\">\n                <div class=\"form-group\">\n                  <select id=\"qty_"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" name=\"item[qty]\" class=\"form-control\" data-qtyitemid=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">\n                    <option value=\"1\">1</option>\n                    <option value=\"2\">2</option>\n                    <option value=\"3\">3</option>\n                    <option value=\"4\">4</option>\n                    <option value=\"5\">5</option>\n                  </select>\n                </div>\n                  <button id=\"btn_"
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\" type=\"submit\" class=\"btn btn-info btn-add-to-cart\" data-btnitemid=\""
    + alias2(alias1(((stack1 = blockParams[0][0]) != null ? stack1.id : stack1), depth0))
    + "\">Add to cart</button>\n                </div>\n            </div>\n          </div>\n          "
    ...
    return "  <div class=\"message\">No items</div>\n";
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.items : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1),"inverse":container.noop,"data":data})) != null ? stack1 : "")
    + "\n";
  ...
  return "\n        <div data-cardDivID=\""
    ...
    + "\">\n          <div class=\"row\">\n            <div class=\"col-xs-4\">\n              <img src=\"https://s3.amazonaws.com/anna0312wdi/nozama-images/"
    + alias4(alias5(((stack1 = (depth0 != null ? depth0.itemId : depth0)) != null ? stack1.image : stack1), depth0))
    + "\" class=\"item-image-cart\">\n            </div>\n            <div class=\"col-xs-8\">\n              <strong>"
    + alias4(alias5(((stack1 = (depth0 != null ? depth0.itemId : depth0)) != null ? stack1.name : stack1), depth0))
    + "</strong>\n              <br /> $"
    + alias4(alias5(((stack1 = (depth0 != null ? depth0.itemId : depth0)) != null ? stack1.price : stack1), depth0))
    + "\n              <br>\n              <div class=\"form-inline\">\n                <div class=\"form-group\">\n                  <input type=\"number\" min=\"1\" class=\"form-control small-input\" id=\"updateqty_"
    + alias4(alias5(((stack1 = (depth0 != null ? depth0.itemId : depth0)) != null ? stack1._id : stack1), depth0))
    + "\" name=\"item[qty]\" data-qtyitemid=\""
    + alias4(alias5(((stack1 = (depth0 != null ? depth0.itemId : depth0)) != null ? stack1._id : stack1), depth0))
    + "\" value=\""
    + alias4(((helper = (helper = helpers.itemQty || (depth0 != null ? depth0.itemQty : depth0)) != null ? helper : alias2),(typeof helper === alias3 ? helper.call(alias1,{"name":"itemQty","hash":{},"data":data}) : helper)))
    + "\">\n                </div>\n                  <button id=\"btnupdateqty_"
    + alias4(alias5(((stack1 = (depth0 != null ? depth0.itemId : depth0)) != null ? stack1._id : stack1), depth0))
    + "\" type=\"submit\" class=\"btn btn-info btn-update-item\" data-btnupdateitemid=\""
    + alias4(alias5(((stack1 = (depth0 != null ? depth0.itemId : depth0)) != null ? stack1._id : stack1), depth0))
    + "\">Update qty</button>\n\n                  <button id=\"btnremove_"
    + alias4(alias5(((stack1 = (depth0 != null ? depth0.itemId : depth0)) != null ? stack1._id : stack1), depth0))
    + "\" class=\"btn btn-danger btn-remove-item\" data-btnremoveitemid=\""
    + alias4(alias5(((stack1 = (depth0 != null ? depth0.itemId : depth0)) != null ? stack1._id : stack1), depth0))
    + "\">X</a>\n                </div>\n            </div>\n          </div>\n          "
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return "<h4>\n Grand total: $"
    + container.escapeExpression(container.lambda(((stack1 = (depth0 != null ? depth0.summary : depth0)) != null ? stack1.totalCost : stack1), depth0))
    + "\n</h4>\n"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = (depth0 != null ? depth0.summary : depth0)) != null ? stack1.totalItems : stack1),{"name":"if","hash":{},"fn":container.program(2, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "");
},"2":function(container,depth0,helpers,partials,data) {
    return "  <input type=\"button\" class=\"btn btn-default\" value=\"Check Out\" id=\"checkout\">\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data) {
    var stack1;
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data,blockParams) {
  ...
  return "        <tbody>\n"
    + ((stack1 = helpers.each.call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.orders : depth0),{"name":"each","hash":{},"fn":container.program(2, data, 1, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "        </tbody>\n";
},"2":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),((stack1 = blockParams[0][0]) != null ? stack1.editable : stack1),{"name":"if","hash":{},"fn":container.program(3, data, 0, blockParams),"inverse":container.noop,"data":data,"blockParams":blockParams})) != null ? stack1 : "");
},"3":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1, alias1=container.lambda, alias2=container.escapeExpression;

  return "          <tr>\n            <td>"
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.orderDate : stack1), depth0))
    + "</td>\n            <td>"
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.id : stack1), depth0))
    + "</td>\n            <td>"
    + alias2(alias1(((stack1 = blockParams[1][0]) != null ? stack1.orderTotal : stack1), depth0))
    + "</td>\n          </tr>\n";
},"5":function(container,depth0,helpers,partials,data) {
    return "          <div class=\"message\">No Orders</div>\n";
},"compiler":[7,">= 4.0.0"],"main":function(container,depth0,helpers,partials,data,blockParams) {
    var stack1;

  return "\n  <div class=\"row\">\n    <div class=\"form-group\">\n      <table id=\"clientOrderHistory\" class=\"table table-striped\">\n        <thead>\n          <tr>\n            <th>Purchase Date</th>\n            <th>Item ID</th>\n            <th>Total Amount</th>\n          </tr>\n        </thead>\n"
    + ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.orders : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0, blockParams),"inverse":container.program(5, data, 0, blockParams),"data":data,"blockParams":blockParams})) != null ? stack1 : "")
    + "      </table>\n    </div>\n  </div>\n";
},"useData":true,"useBlockParams":true});
...
module.exports = (Handlebars["default"] || Handlebars).template({"1":function(container,depth0,helpers,partials,data) {
  ...
  return ((stack1 = helpers["if"].call(depth0 != null ? depth0 : (container.nullContext || {}),(depth0 != null ? depth0.cart : depth0),{"name":"if","hash":{},"fn":container.program(1, data, 0),"inverse":container.noop,"data":data})) != null ? stack1 : "");
},"useData":true});
